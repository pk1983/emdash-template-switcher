...
export async function runAdd({ cwd, name, force }) {
	...
	if (isTemplatePackageSpec(name)) {
		// 1. install a package archive (marketplace / local tarball)
		log.step("1. Install template package");
		const manifest = await installTemplatePackage({ cwd, spec: name, force });
		templateName = manifest.id;
	} else {
	...
	if (registerTemplateInRegistry(join(templateDir, "index.ts"), templateName))
	...
	log.step("Done ✓");
	console.log(`
  Edit your new template in:  src/template/${templateName}/
